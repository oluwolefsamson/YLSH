...
const AdminOverviewPage: NextPageWithLayout = () => {
  ...
  const statCards = [
    {
      ...
      progress: statsLoading ? undefined : totalUsers ? (verifiedUsers / totalUsers) * 100 : 0,
      icon: <Users size={20} />,
      change: statsLoading ? undefined : `${verifiedUsers} verified`,
    },
    {
      label: "Active events",
      value: statsLoading ? "..." : String(stats?.activeEvents ?? 0),
      progress: undefined,
      icon: <CalendarCheck size={20} />,
    },
    {
      label: "Certificates issued",
      value: statsLoading ? "..." : String(totalCerts),
      progress: statsLoading ? undefined : totalUsers ? Math.min((totalCerts / totalUsers) * 100, 100) : 0,
      icon: <Award size={20} />,
      change: statsLoading ? undefined : `of ${totalUsers} users`,
    },
    {
      label: "NIN verified",
      value: statsLoading ? "..." : String(verifiedUsers),
      progress: statsLoading ? undefined : totalUsers ? (verifiedUsers / totalUsers) * 100 : 0,
      icon: <ShieldCheck size={20} />,
      change: statsLoading ? undefined : totalUsers ? `${Math.round((verifiedUsers / totalUsers) * 100)}% rate` : undefined,
    },
  ]
  ...
  const eventSkeleton = (
    <div className="flex flex-col gap-3">
      {[1, 2, 3].map((i) => (
        <div key={i} className="p-3 rounded-xl border border-slate-200/18 animate-pulse">
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {statCards.map((card) => (
          <StatCard key={card.label} label={card.label} value={card.value} icon={card.icon} progress={card.progress} change={card.change} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[7fr_5fr] gap-5">
        ...
        <div className="flex flex-col gap-5">
                ...
                {recentEvents.map((event) => (
                  <div key={event._id} className="p-3 rounded-xl border border-slate-200/18">
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      ...
                      <span className={cn("text-xs font-bold px-2 py-0.5 rounded-full", event.status === "upcoming" ? "bg-blue-100 text-[#082F49]" : "bg-muted text-muted-foreground")}>{event.status}</span>
                    </div>
                    ...
                    {event.capacity ? (
                      ...
                      <p className="text-xs text-muted-foreground">{event.registeredCount} registered · Unlimited</p>
                    )}
                  </div>
                ))}
          ...
          <div className="p-5 rounded-2xl text-white" style={{ background: "linear-gradient(135deg, rgba(6,30,53,0.98) 0%, rgba(8,47,73,0.98) 100%)", boxShadow: "0 12px 30px rgba(15,23,42,0.06)" }}>
