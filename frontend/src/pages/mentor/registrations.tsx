...
const statusConfig = {
  registered: { label: 'Confirmed', color: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircle },
  waitlisted: { label: 'Waitlisted', color: 'bg-amber-100 text-amber-700 border-amber-200', icon: Clock3 },
  attended: { label: 'Attended', color: 'bg-primary/10 text-primary border-primary/20', icon: CheckCircle },
  cancelled: { label: 'Cancelled', color: 'bg-slate-100 text-slate-500 border-slate-200', icon: XCircle },
}

const MentorRegistrationsPage: NextPageWithLayout = () => {
                ...
                <div key={reg._id} className="rounded-2xl border border-slate-200/18 overflow-hidden">
                  <div className="p-4 md:p-5 flex items-start gap-4 flex-wrap">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        {reg.event?.category && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-primary/10 text-primary">{reg.event.category}</span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        {reg.event?.date && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-sm text-muted-foreground">{new Date(reg.event.date).toLocaleDateString('en-NG', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</span>
                            {reg.event.time && <><Clock size={13} className="text-muted-foreground ml-1" /><span className="text-sm text-muted-foreground">{reg.event.time}</span></>}
                          </div>
                        )}
                        {reg.event?.venue && (
                          <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {showQR && (
                        <button onClick={() => setExpandedId(isExpanded ? null : reg._id)} className="px-4 py-1.5 rounded-full bg-primary text-white text-sm font-semibold hover:bg-[#0d5c54] transition-colors">
                  ...
                  {reg.status === 'waitlisted' && (
                    <div className="px-5 pb-4">
                      <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                        <p className="text-sm text-amber-700 font-medium">You're on the waitlist. You'll be notified automatically if a spot opens up.</p>
                      </div>
                    </div>
                  )}
                  ...
                  {showQR && isExpanded && (
                    <div className="border-t border-slate-200/18 p-6 flex flex-col items-center" style={{ backgroundColor: 'rgba(18,124,113,0.03)' }}>
                      ...
                      <button onClick={() => handleDownloadQR(reg.qrToken, reg.event?.title ?? '')} className="flex items-center gap-2 px-5 py-2 rounded-full border border-border text-sm font-semibold hover:bg-muted transition-colors">
                        <Download size={14} />
                        Download QR
                      </button>
                    </div>
...
MentorRegistrationsPage.getLayout = (page) => <MentorLayout>{page}</MentorLayout>
export default MentorRegistrationsPage
