...
const events: SuperAdminEvent[] = [
  ...
  { id: 4, title: 'Climate Action Workshop', date: 'May 10, 2026', venue: 'University of Lagos', category: 'Workshop', capacity: 200, registered: 185, certificates: 170, status: 'past' },
  { id: 5, title: 'Youth Innovation Forum', date: 'Apr 5, 2026', venue: 'Eko Hotel, Lagos', category: 'Forum', capacity: 400, registered: 380, certificates: 372, status: 'past' },
]

const categoryBadge: Record<string, string> = { Summit: 'bg-primary/10 text-primary', Workshop: 'bg-green-100 text-green-700', Masterclass: 'bg-amber-100 text-amber-700', Forum: 'bg-muted text-muted-foreground' }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <StatCard label="Total registered" value={events.reduce((s, e) => s + e.registered, 0).toLocaleString()} icon={<Users size={20} />} progress={82} accent="#f59e0b" />
      </div>
            ...
            <div key={event.id} className="p-4 rounded-xl border border-slate-200/18">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {event.status === 'past' && event.certificates > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">{event.certificates} certificates issued</p>
                  )}
                </div>
