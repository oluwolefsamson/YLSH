...
import { CalendarCheck, Calendar, MapPin, Users, Plus, X, Loader2, Search, Download, Eye, Trash2 } from 'lucide-react'
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#7c3aed] text-white text-sm font-bold shadow-md hover:bg-[#6d28d9] transition-colors">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((event) => {
                    const progress = event.capacity ? Math.min((event.registeredCount / event.capacity) * 100, 100) : 100
                    return (
                      <tr key={event._id} className="transition-colors hover:bg-slate-50/60">
                        ...
                        <td className="px-4 py-3.5">
                          <span className={cn('inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold', categoryBadge[event.category] ?? 'bg-muted text-muted-foreground')}>
                            {event.category}
                          </span>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className={cn('inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold capitalize border', event.status === 'upcoming' || event.status === 'ongoing' ? 'border-blue-200 bg-blue-50 text-[#082F49]' : 'border-slate-200 bg-slate-50 text-slate-600')}>
                            {event.status}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3.5 min-w-[220px]">
                          <div className="flex items-center justify-between gap-3 mb-1">
                            ...
                            <span className="text-xs text-muted-foreground whitespace-nowrap">
                              {event.capacity ? `${event.registeredCount}/${event.capacity}` : `${event.registeredCount} registered · Unlimited`}
                            </span>
                          </div>
                          {event.capacity ? (
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              onClick={() => setViewId(event._id)}
                              title="View details"
                              className="w-8 h-8 flex items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-primary hover:border-primary/40 transition-colors"
                            >
                              <Eye size={14} />
                            </button>
                            <button
                              ...
                              title="Export CSV"
                              className="w-8 h-8 flex items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-primary hover:border-primary/40 transition-colors disabled:opacity-50"
                            >
                              {exportingId === event._id ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
                            </button>
                            <button
                              onClick={() => setConfirmDelete({ id: event._id, title: event.title })}
                              title="Delete event"
                              className="w-8 h-8 flex items-center justify-center rounded-md border border-red-200 text-red-600 hover:bg-red-50 transition-colors"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
