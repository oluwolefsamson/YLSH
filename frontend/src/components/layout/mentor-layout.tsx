...
const MentorLayout: FC<Props> = ({ children }) => {
  ...
  if (user && (user.role !== 'mentor' || user.approvalStatus !== 'approved')) return null

  const initials = user ? `${user.firstName[0]}${user.lastName[0]}`.toUpperCase() : 'M'
  const fullName = user ? `${user.firstName} ${user.lastName}` : 'Mentor'
          ...
          <header className="sticky top-0 z-30 bg-white border-b border-slate-200 h-16 px-4 md:px-6 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:block">
                <p className="text-sm font-semibold text-slate-800">Mentor Portal</p>
